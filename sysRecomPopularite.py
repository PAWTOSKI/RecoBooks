...
def calculRatingByBook(dfRatings):
    """
    ...
    """
    ratings_by_livre = pd.DataFrame(dfRatings
...
def note_ponderee( x, mseuil, cmoyen):
    """
        caculer 
    """
    v = x['cnt_vote']
...
def populariteNotePonderer(dfRatings: pd.DataFrame, nbLivres=30):
    """
        ...
        *** Return:
        Dataframe de book_id avec son rating moyen et son nombre de fois de vote.
    """
    ratingByBook = calculRatingByBook(dfRatings)
    ...
    q_ratings_livre['score'] = q_ratings_livre.apply(lambda x:  note_ponderee(x, nb_seuil, c_moyen), axis=1)
    ...
    result = q_ratings_livre.head(nbLivres)[['book_id', 'score']].copy()
